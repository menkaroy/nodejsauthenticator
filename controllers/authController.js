...
exports.registerHandle = async (req, res) => {
  ...
  try {
    ...
    req.flash(
      "error_msg",
      error.message || "Something went wrong on our end. Please register again."
    );
...
exports.activateHandle = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      throw new Error("Email ID already registered! Please log in.");
    }
    ...
    req.flash(
      "error_msg",
      error.message || "Incorrect or expired link! Please register again."
    );
    res.redirect("/auth/register");
  }
};
...
//     // Rest of your code for sending the password reset link
//   } catch (error) {
//     console.error(error);
//     req.flash(
//       "error_msg",
...
exports.gotoReset = async (req, res) => {
  ...
  if (token) {
    try {
      ...
      const { _id } = decodedToken;
      const user = await User.findById(_id).exec(); // Use async/await here
...
exports.resetPassword = async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    req.flash("error_msg", error.message || "Error resetting password!");
    res.redirect(`/auth/reset/${id}`);
  }
};
//------------ Login Handle ------------//
